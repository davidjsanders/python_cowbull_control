function deployK8Scomponents {
    ...
    then
        ...
        kubectl create configmap cowbull-config \
          --from-file=cowbull.cfg="configuration/cowbull-"$persister".cfg"
        echo
        ...
        echo "-------------------------"
        echo "kubectl create configmap cowbull-config --from-file=cowbull.cfg=configuration/cowbull-"$persister".cfg"
        if (( $MONGODB==1 ))
        then
